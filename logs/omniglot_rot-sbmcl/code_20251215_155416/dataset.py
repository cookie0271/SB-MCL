...
def safe_random_sample(population, k):
    global _WARNED_SAMPLE_SIZE
    if len(population) == 0:
        raise ValueError("样本池为空，无法采样！")
    if k <= 0:
        raise ValueError("采样数k必须为正整数！")

    if len(population) >= k:
        return random.sample(population, k)
    else:
        # 仅首次触发时打印警告
        if not _WARNED_SAMPLE_SIZE:
            print(f"警告：检测到样本数不足（样本池{len(population)} < 目标{k}），后续将自动补全（仅提示一次）")
            _WARNED_SAMPLE_SIZE = True
        sampled = population.copy()
        while len(sampled) < k:
            need = k - len(sampled)
