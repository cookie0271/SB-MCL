    ...
    for (tasks, shots), meta_split in product(eval_settings, meta_splits):
        ...
        with torch.no_grad(), Timer('Evaluation time: {:.3f}s'):
            ...
            if rank == 0:  # 仅主进程记录
                ...
                # 方法1：从output中提取准确率（适配你的Output类结构）
                # 先打印output的所有键，确认准确率字段名（仅首次运行需要）
                print("Output所有字段：", output.keys())

                # 核心：根据Output实际字段提取准确率（以下是通用适配）
                accuracy = None
                # 情况1：如果Output中有'acc/meta_test'字段（分类任务常见）
                if 'acc/meta_test' in output:
                    accuracy = output['acc/meta_test'].mean().item()
                # 情况2：如果只有loss，先记录loss（确保TensorBoard有数据）
                elif 'loss/meta_test' in output:
                    loss = output['loss/meta_test'].mean().item()
                    writer.add_scalar(f'evaluation/{tasks}t{shots}s/loss', loss, config['max_train_steps'])
                # 情况3：从classification_summary中提取（如果有）
                elif 'classification_summary/meta_test' in output:
                    accuracy = output['classification_summary/meta_test']['accuracy'].item()

                # 记录准确率到TensorBoard（有值才记录）
